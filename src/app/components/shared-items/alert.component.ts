  ...
  <div *ngIf="alerts.length != 0" class="sticky-top">
    <div *ngFor="let alert of alerts" class="{{cssClass(alert)}}">
      <ng-container *ngIf="alert.message">
        ...
        <br>
        <div class="info-dropdown" *ngIf="alert.id == getSelectedId()">
          <pre>{{alert.message}}</pre>
          <button class="btn badge btn-outline-secondary btn-sm btn-copy" *ngIf="alert.message" [cdkCopyToClipboard]="alert.message">
            Копировать
          </button>
        </div>
      </ng-container>
