...
/** Страница для отображения ошибок. Берёт информацию о последней ошибке из errors.service */
@Component({
  selector: 'error',
  ...
  template: `
    ...
    <div *ngIf="error.Info" class="info-dropdown">
      ...
      <div class="card" *ngIf="!hidden">
        ...
        <button class="btn btn-outline-primary btn-sm btn-copy" *ngIf="error.Info" [cdkCopyToClipboard]="error.Info">
          Копировать
        </button>
      </div>
    </div>
