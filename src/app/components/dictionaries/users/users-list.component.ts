...
/** Страница со списком пользователей. Поддерживает фильтрациию и пагинацию. 
* @param {number} page - номер страницы пагинатора
* @param {number} pageSize - количество элементов на странице
* @param {number} positionId - Id должности (по умолчанию -1 - без фильтра)
*/
@Component({
  selector: 'users-list',
...
export class UsersListComponent implements OnInit {
  ...
  positions?: Position[];
  displayedColumns = ['UserName', 'Email', 'Position', 'Company', 'Permissions', 'Edit'];
  ...
  addUser(){
    ...
    const dialogRef = this.dialog.open(NewUserDialog, {
      data: {user: new User("", "", ""), positions: this.positions}
    });
  ...
  removeUser(cwid: string) {
    this.selected = new User("", "");
    this.userSvc.deleteUser(cwid).subscribe(() => {
      this.alertSvc.info("Пользователь удалён", {closeTime: 5000, single: true});
      this.users = this.users?.filter(user => user.CWID != cwid);
      if (this.totalElements != 0)  this.totalElements--;
    });
  }

  beginEdit(user: User){
    this.selected.CWID = user.CWID;
    ...
    this.selected.LeadingSubgroup = user.LeadingSubgroup;
    this.selected.OrgName = user.OrgName;
    this.selected.CWID = user.CWID;
    this.selected.ExternalCompany = user.ExternalCompany;
    this.selected.CompanyCode = user.CompanyCode;
    this.selected.ManagerCWID = user.ManagerCWID;
    this.selected.EmployeeType = user.EmployeeType;
  }
  ...
  reset(user: User){
    ...
    user.CWID = this.selected.CWID;
    user.CompanyCode = this.selected.CompanyCode;
    user.ExternalCompany = this.selected.ExternalCompany;
    user.LeadingSubgroup = this.selected.LeadingSubgroup;
    user.OrgName = this.selected.OrgName;
    user.ManagerCWID = this.selected.ManagerCWID;
    user.EmployeeType = this.selected.EmployeeType;
    this.selected = new User("", "");
  }
